...
void Block::MineBlock(uint32_t difficulty_rate)
{
    ...
    // the front portion of the Hash is then compared the string of zeros we’ve just created; if no match is found the loop is repeated until a match is found.
   // Once a match is found a message is sent to the output buffer to say that the block has been successfully mined
 ...
 // usually function that takes less memory we make them inlline
...
/*
stringstream is just like cout except that the stream is being captured in a buffer. (Actually, cout technically writes to a buffer also, but it gets flushed whenever the system decides to flush it.)
You use it in exactly the same way you do cout, and when you are ready to use its contents, you can call one of its methods, the most common of which is str(), which produces a string object.
*/
...
//As a blocks are added to the blockchain they need to reference the previous block using its Hash,
//but as the blockchain must start somewhere we have to create a block for the next block to reference, we call this a genesis block. A genesis block is created and placed onto the blockchain vector (line 2).
//We then set the _difficulty_rate level (line 3) depending on how hard we want to make the PoW process.
...
{
    vector<int>v={1,2};
    v.emplace_back(3); //123

    vector<string>vs;
    vs.emplace_back("My");
    vs.emplace_back("name is");
    vs.emplace_back("Golam");
